(function () {
  ...
  function activate(name, updateHash, scroll) {
    ...
    revealActiveTab();
    if (updateHash) {
      try { if (location.hash !== '#' + name) location.hash = name; } catch (_) {}
    }
  ...
  function routeHash(scroll) {
    ...
    key = pageKey(key);
    if (names.has(key)) return activate(key, false, scroll);
  ...
  root.addEventListener('click', event => {
    ...
    if (panel) {
      event.preventDefault();
      activate(panel.dataset.page, false, false);
  ...
  if (homeNews) {
    homeNews.innerHTML = `
      <div class="home-section-head"><h2>News</h2></div>
      <div class="news-archive">
        <h3 class="news-year">2026</h3>
        <ul class="news-list">
          <li class="news-item"><time datetime="2026-09">2026.09</time><p>2026학년도 최우수 연구업적 교원 (동아대학교) <a href="https://www.donga.ac.kr/kor/CMS/Board/Board.do?mCode=MN044&amp;mode=view&amp;mgr_seq=54&amp;board_seq=8516311" rel="noopener noreferrer" target="_blank">News Link ↗</a></p></li>
          <li class="news-item"><time datetime="2026-09">2026.09</time><p>Otim Kelvin Kennedy (M.S. graduate) Lecturer 임용 (Uganda Christian University, Department of Engineering and Environment)</p></li>
        ...
        <ul class="news-list"><li class="news-item"><time datetime="2025-03">2025.03</time><p>변예림 학생 석사과정 입학</p></li></ul>
        ...
        <ul class="news-list"><li class="news-item"><time datetime="2023-03">2023.03</time><p>동아대학교 건설시스템공학과 조교수 임용</p></li></ul>
      </div>`;
  }


  // news-item-dedupe-20260919: remove duplicate news rows if any stale/injected markup remains.
  if (homeNews) {
    const seenNews = new Set();
    homeNews.querySelectorAll('.news-item').forEach(item => {
      const key = item.textContent.replace(/\s+/g, ' ').trim();
      if (seenNews.has(key)) item.remove();
      else seenNews.add(key);
    });
  }
  ...
  if (publications) {
    ...
    if (year2026) {
      const span = year2026.querySelector('h2 span');
      if (span) span.textContent = '8 entries';
    }
    ...
    Array.from(publications.querySelectorAll('.pub-source p')).forEach(p => {
  ...
  if (search) {
    ...
    function filter() {
      ...
      groups.forEach(group => {
        group.hidden = !Array.from(group.querySelectorAll('.paper')).some(p => !p.hidden);
      });
      manuscriptEntries.forEach(entry => { entry.hidden = false; });
      if (reviews) reviews.hidden = false;
      root.querySelectorAll('.milestone').forEach(node => { node.hidden = year !== 'all' || words.length > 0; });
      if (count) count.textContent = journals + ' of ' + journalEntries.length + ' journal entries · ' + manuscriptEntries.length + ' manuscript entries';
  ...
  // Load the Google Drive gallery module without changing index.html.
  if (!document.querySelector('script[data-jeonlab-gallery]')) {
    const galleryScript = document.createElement('script');
    galleryScript.src = './assets/js/photo-gallery.js';
    galleryScript.defer = true;
    galleryScript.dataset.jeonlabGallery = 'true';
